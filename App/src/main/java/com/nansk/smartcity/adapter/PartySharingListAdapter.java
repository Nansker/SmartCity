...
public class PartySharingListAdapter extends RecyclerView.Adapter<PartySharingListAdapter.BodyViewHolder> {
    private Context context;
    private List<DesignSharingBean> data;
    private OnItemCallBack onItemCallBack;

    int windowWidth;
    public PartySharingListAdapter(Context context) {
        this.context = context;
        windowWidth = WindowMangerUtils.getWindowSize(context, 0)-200;
    }

    public List<DesignSharingBean> getData() {
        return data;
    }

    public void setData(List<DesignSharingBean> data) {
    ...
    public void onBindViewHolder(@NonNull BodyViewHolder holder, final int position) {
        final DesignSharingBean bean = data.get(position);
        holder.nickNameTv.setText(bean.getNickName());
        holder.createTimeTv.setText(bean.getDate());
